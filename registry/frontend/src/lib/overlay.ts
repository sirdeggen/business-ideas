...
export interface OverlayRegister extends RegisterRecord {
  txid: string
  outputIndex: number
}
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): RegistryPayload | null {
  for (const position of ['before', 'after'] as const) {
    try {
      const item = parseRegistryFields(PushDrop.decode(lockingScript, position).fields)
      if (item) return item
    } catch {
...
function registryOutputIndexes(tx: Transaction): number[] {
...
async function submitBeefFallback(host: string, topic: string, beef: number[]): Promise<SubmitResult> {
  ...
  return {
    admitted: registryOutputIndexes(tx),
    raw,
    host,
    topic
  }
...
export async function submitRegistryTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupRegisterItems(base: string, query: RegisterQuery = {}): Promise<OverlayItem[]> {
  const host = overlayUrl(base)
...
export async function lookupRegister(
  ...
  const items = await lookupRegisterItems(base, { registerId, txid })
  return viewFromItems(items, registerId)
}

export function viewFromItems(items: OverlayItem[], registerId?: string): RegisterView {
  const scoped = registerId
    ? items.filter((item) => item.payload.registerId === registerId)
    : items
  const registers = scoped
    .filter((item): item is OverlayItem & { payload: RegisterRecord } => item.payload.kind === 'register')
    .map((item) => ({ ...item.payload, txid: item.txid, outputIndex: item.outputIndex }))
  const issues = scoped
    .filter((item): item is OverlayItem & { payload: IssueRecord } => item.payload.kind === 'issue')
    .map((item) => item.payload)
  const transfers = scoped
    .filter((item): item is OverlayItem & { payload: TransferRecord } => item.payload.kind === 'transfer')
    .map((item) => item.payload)
  const register = latestRegister(registers) as OverlayRegister | null
  const fold = register
    ? foldRegister(register, issues, transfers)
    : { holdings: [], issuedUnits: 0, acceptedTransfers: 0, skippedIssues: 0, skippedTransfers: 0 }

  return {
    registerId: registerId || register?.registerId || '',
    register,
    issues,
    transfers,
    fold
...
function matchesQuery(item: OverlayItem, query: RegisterQuery): boolean {
  ...
  if (query.registerId && item.payload.registerId !== query.registerId) return false
  return true
}
