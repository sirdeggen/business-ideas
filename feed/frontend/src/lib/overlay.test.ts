...
describe('overlay topic rails', () => {
  ...
  it('does not invent a custom topic on localhost', () => {
    ...
    expect(overlayLookupService('http://127.0.0.1:5184')).toBe('ls_anytx')
    expect(overlayTopic('http://[::1]:5184')).toBe('tm_anytx')
    expect(overlayTopic('http://localhost:5184')).not.toBe('tm_feed')
    expect(overlayLookupService('http://localhost:5184')).not.toBe('ls_feed')
  })
})
