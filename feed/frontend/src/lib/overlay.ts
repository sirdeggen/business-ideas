...
export interface OverlaySub extends FeedSub {
  txid: string
  outputIndex: number
}

export interface OverlayReading extends FeedReading {
  txid: string
  outputIndex: number
}
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): FeedPayload | null {
  for (const position of ['before', 'after'] as const) {
    try {
      const item = parseFeedFields(PushDrop.decode(lockingScript, position).fields)
      if (item) return item
    } catch {
...
function feedOutputIndexes(tx: Transaction): number[] {
...
export async function submitFeedTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupDesk(base: string): Promise<DeskSnapshot> {
  const host = overlayUrl(base)
  const service = overlayLookupService(host)
  const answers = await queryAnytx(host, service)
  const items = answers.flatMap(itemsFromAnswer).filter((item) => item.payload.magic === MAGIC)
  const rawFeeds = items.flatMap((item) => (
    item.payload.kind === 'feed'
      ? [{ feed: item.payload, txid: item.txid, outputIndex: item.outputIndex }]
      : []
  ))
  const pulses = items.flatMap((item) => (item.payload.kind === 'pulse' ? [item.payload] : []))
  const unique = dedupeFeeds(rawFeeds.map((row) => row.feed))
  return {
    feeds: unique.map((feed) => {
      const row = rawFeeds.find((item) => (
        item.feed.feedId === feed.feedId && item.feed.timestamp === feed.timestamp
      )) ?? rawFeeds.find((item) => item.feed.feedId === feed.feedId)
      const commitment = latestCommitment(feed, pulses)
      return {
        feed,
        txid: row?.txid ?? '',
        outputIndex: row?.outputIndex ?? 0,
        updatedAt: commitment.timestamp,
        valueHash: commitment.valueHash
      }
    }),
    subs: items.flatMap((item) => (
      item.payload.kind === 'sub'
        ? [{ ...item.payload, txid: item.txid, outputIndex: item.outputIndex }]
        : []
    )),
    pulses,
    readings: items.flatMap((item) => (
      item.payload.kind === 'reading'
        ? [{ ...item.payload, txid: item.txid, outputIndex: item.outputIndex }]
        : []
    )),
    queries: items.flatMap((item) => (item.payload.kind === 'query' ? [item.payload] : []))
  }
}

async function queryAnytx(host: string, service: string): Promise<LookupAnswer[]> {
...
function fromContext(context: number[] | undefined, outputIndex: number): OverlayItem | null {
  ...
  try {
    const parsed = JSON.parse(new TextDecoder().decode(Uint8Array.from(context))) as FeedPayload & {
      txid?: string
      outputIndex?: number
    }
    if (parsed.magic !== MAGIC || !parsed.kind) return null
    const checked = parseFeedFields(encodePayload(parsed))
...
function encodePayload(payload: FeedPayload): number[][] {
  if (payload.kind === 'feed') return encodeFeedFields(payload)
  if (payload.kind === 'pulse') return encodePulseFields(payload)
  if (payload.kind === 'sub') return encodeSubFields(payload)
  if (payload.kind === 'query') return encodeQueryFields(payload)
  return encodeReadingFields(payload)
}
