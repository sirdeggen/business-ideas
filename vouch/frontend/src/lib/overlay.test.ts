...
describe('overlay topic rails', () => {
  ...
  it('does not invent a custom topic on localhost', () => {
    ...
    expect(overlayLookupService('http://127.0.0.1:5183')).toBe('ls_anytx')
    expect(overlayTopic('http://[::1]:5183')).toBe('tm_anytx')
    expect(overlayTopic('http://localhost:5183')).not.toBe('tm_vouch')
    expect(overlayLookupService('http://localhost:5183')).not.toBe('ls_vouch')
  })
})
