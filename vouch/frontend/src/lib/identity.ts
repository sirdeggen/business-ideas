import { isIdentityKey } from '../../../protocol/vouch'

const RESOLVE_MS = 4000
const FACE_FALLBACK = 'Known party'

export function faceName(resolved: string | null | undefined, fallback = FACE_FALLBACK): string {
...
export async function resolveIdentity(input: string): Promise<string> {
  ...
  if (!trimmed) throw new Error('Write a name or an account.')
