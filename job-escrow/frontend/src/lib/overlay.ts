...
export interface OverlayFund extends JobFund {
  txid: string
  outputIndex: number
}

export interface OverlaySubmit extends JobSubmit {
  txid: string
  outputIndex: number
}

export interface OverlayRelease extends JobRelease {
  txid: string
  outputIndex: number
}

export interface OverlayChallenge extends JobChallenge {
  txid: string
  outputIndex: number
}

export interface OverlayRefund extends JobRefund {
  txid: string
  outputIndex: number
}
...
export interface JobView {
  ...
  submit: OverlaySubmit | null
  release: OverlayRelease | null
  challenge: OverlayChallenge | null
  refund: OverlayRefund | null
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): JobPayload | null {
...
function jobOutputIndexes(tx: Transaction): number[] {
...
export async function submitJobTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupJobItems(base: string, query: JobQuery = {}): Promise<OverlayItem[]> {
  const host = overlayUrl(base)
...
export async function lookupJob(
  ...
  const items = await lookupJobItems(base, { jobId, txid })
  return viewFromItems(items, jobId)
}

export function viewFromItems(items: OverlayItem[], jobId?: string): JobView {
  const scoped = jobId
    ? items.filter((item) => item.payload.jobId === jobId)
    : items
  const funds = scoped.filter((item) => item.payload.kind === 'fund') as Array<OverlayItem & { payload: JobFund }>
  const submits = scoped.filter((item) => item.payload.kind === 'submit') as Array<OverlayItem & { payload: JobSubmit }>
  const releases = scoped.filter((item) => item.payload.kind === 'release') as Array<OverlayItem & { payload: JobRelease }>
  const challenges = scoped.filter((item) => item.payload.kind === 'challenge') as Array<OverlayItem & { payload: JobChallenge }>
  const refunds = scoped.filter((item) => item.payload.kind === 'refund') as Array<OverlayItem & { payload: JobRefund }>

  const fund = funds[0]
    ? { ...funds[0].payload, txid: funds[0].txid, outputIndex: funds[0].outputIndex }
    : null
  const submit = submits[0]
    ? { ...submits[0].payload, txid: submits[0].txid, outputIndex: submits[0].outputIndex }
    : null
  const release = releases[0]
    ? { ...releases[0].payload, txid: releases[0].txid, outputIndex: releases[0].outputIndex }
    : null
  const challenge = challenges[0]
    ? { ...challenges[0].payload, txid: challenges[0].txid, outputIndex: challenges[0].outputIndex }
    : null
  const refund = refunds[0]
    ? { ...refunds[0].payload, txid: refunds[0].txid, outputIndex: refunds[0].outputIndex }
    : null

  return {
    jobId: jobId || fund?.jobId || '',
    fund,
    submit,
    release,
    challenge,
    refund,
    status: jobStatus({ fund, submit, release, challenge, refund })
  }
}
 ...
 * BEEF output and keep MAGIC jobescrow fields only.
 */
...
function matchesQuery(item: OverlayItem, query: JobQuery): boolean {
  ...
  if (query.jobId && item.payload.jobId !== query.jobId) return false
  return true
}
