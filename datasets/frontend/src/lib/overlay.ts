...
export interface OverlayListing extends DatasetListing {
  txid: string
  outputIndex: number
}

export interface OverlayReceipt extends DatasetReceipt {
  txid: string
  outputIndex: number
}
...
/** Public and local both use tm_anytx — this stall does not invent a custom topic. */
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): DatasetPayload | null {
  for (const position of ['before', 'after'] as const) {
    try {
      const item = parseDatasetFields(PushDrop.decode(lockingScript, position).fields)
      if (item) return item
    } catch {
...
function datasetOutputIndexes(tx: Transaction): number[] {
...
async function submitBeefFallback(host: string, topic: string, beef: number[]): Promise<SubmitResult> {
  ...
  return {
    admitted: datasetOutputIndexes(tx),
    raw,
    host,
    topic
  }
...
export async function submitDatasetTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupDatasetItems(base: string, query: DatasetQuery = {}): Promise<OverlayItem[]> {
  const host = overlayUrl(base)
...
export async function lookupListings(base: string): Promise<OverlayListing[]> {
  const items = await lookupDatasetItems(base)
  return items
    .filter((item): item is OverlayItem & { payload: DatasetListing } => item.payload.kind === 'listing')
    .map((item) => ({
      ...item.payload,
      txid: item.txid,
...
function matchesQuery(item: OverlayItem, query: DatasetQuery): boolean {
...
function fromContext(context: number[] | undefined, outputIndex: number): OverlayItem | null {
  ...
  try {
    const parsed = JSON.parse(new TextDecoder().decode(Uint8Array.from(context))) as DatasetPayload & {
      txid?: string
      outputIndex?: number
    }
