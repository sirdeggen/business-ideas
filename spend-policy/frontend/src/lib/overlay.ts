...
import {
  ...
  type PolicyPayload,
  type SpendPayload,
  type SpendPolicyPayload
} from '../../../protocol/spendpolicy'
import { PUBLIC_LOOKUP, PUBLIC_TOPIC } from './config'

export interface OverlayItem {
  payload: SpendPolicyPayload
  txid: string
  outputIndex: number
}

export interface OverlayPolicy extends PolicyPayload {
  txid: string
  outputIndex: number
}

export interface OverlaySpend extends SpendPayload {
  txid: string
  outputIndex: number
}

export interface PolicyView {
  policy: OverlayPolicy | null
  spends: OverlaySpend[]
}
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): SpendPolicyPayload | null {
  for (const position of ['before', 'after'] as const) {
    try {
      const item = parseSpendPolicyFields(PushDrop.decode(lockingScript, position).fields)
      if (item) return item
    } catch {
...
function policyOutputIndexes(tx: Transaction): number[] {
...
async function submitBeefFallback(host: string, topic: string, beef: number[]): Promise<SubmitResult> {
  ...
  return {
    admitted: policyOutputIndexes(tx),
    raw,
    host,
    topic
  }
...
export async function submitPolicyTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupPolicyItems(base: string, query: PolicyQuery = {}): Promise<OverlayItem[]> {
  const host = overlayUrl(base)
...
export async function lookupPolicy(base: string, policyId: string, txid?: string): Promise<PolicyView> {
  const items = await lookupPolicyItems(base, { policyId, txid })
  return viewFromItems(items, policyId)
}

export function viewFromItems(items: OverlayItem[], policyId?: string): PolicyView {
  const scoped = policyId
    ? items.filter((item) => item.payload.policyId === policyId)
    : items
  const policies = scoped.filter((item) => item.payload.kind === 'policy') as Array<OverlayItem & { payload: PolicyPayload }>
  const spends = scoped.filter((item) => item.payload.kind === 'spend') as Array<OverlayItem & { payload: SpendPayload }>
  spends.sort((a, b) => a.payload.spentAt.localeCompare(b.payload.spentAt))
  return {
    policy: policies[0]
      ? { ...policies[0].payload, txid: policies[0].txid, outputIndex: policies[0].outputIndex }
      : null,
    spends: spends.map((item) => ({
      ...item.payload,
      txid: item.txid,
...
function itemsFromAnswer(answer: LookupAnswer): OverlayItem[] {
  ...
  return answer.outputs.flatMap((output) => {
    const fromScript = itemFromBeef(output.beef, output.outputIndex, output.txid)
    return fromScript ? [fromScript] : []
  })
}

function itemFromBeef(beef: number[] | undefined, outputIndex: number, txidHint?: string): OverlayItem | null {
...
function matchesQuery(item: OverlayItem, query: PolicyQuery): boolean {
  ...
  if (query.policyId && item.payload.policyId !== query.policyId) return false
  return true
}
