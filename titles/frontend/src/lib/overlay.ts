...
export interface OverlayTitle extends TitleToken {
  txid: string
  outputIndex: number
}

export interface OverlayExport extends TitleExport {
  txid: string
  outputIndex: number
}
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): TitlePayload | null {
...
function titleOutputIndexes(tx: Transaction): number[] {
...
async function submitBeefFallback(host: string, topic: string, beef: number[]): Promise<SubmitResult> {
  ...
  return {
    admitted: titleOutputIndexes(tx),
    raw,
    host,
    topic
  }
...
export async function submitTitleTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupTitleItems(base: string, query: TitleQuery = {}): Promise<OverlayItem[]> {
  const host = overlayUrl(base)
...
export async function lookupTitles(base: string): Promise<OverlayTitle[]> {
  const items = await lookupTitleItems(base)
  const titles = items
    .filter((item): item is OverlayItem & { payload: TitleToken } => item.payload.kind === 'title')
    .map((item) => ({
      ...item.payload,
      txid: item.txid,
      outputIndex: item.outputIndex
    }))
  return currentTitles(titles)
}
...
function matchesQuery(item: OverlayItem, query: TitleQuery): boolean {
  ...
  if (query.titleId && item.payload.titleId !== query.titleId) return false
  return true
}
...
function fromContext(context: number[] | undefined, outputIndex: number): OverlayItem | null {
  ...
  try {
    const parsed = JSON.parse(new TextDecoder().decode(Uint8Array.from(context))) as TitlePayload & {
      txid?: string
      outputIndex?: number
    }
