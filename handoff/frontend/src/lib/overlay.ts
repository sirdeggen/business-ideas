...
export interface OverlayList extends HandoffList {
  txid: string
  outputIndex: number
}

export interface OverlayFund extends HandoffFund {
  txid: string
  outputIndex: number
}

export interface OverlayConfirm extends HandoffConfirm {
  txid: string
  outputIndex: number
}

export interface OverlayRelease extends HandoffRelease {
  txid: string
  outputIndex: number
}
...
export interface ListingView {
  listingId: string
  list: OverlayList | null
  fund: OverlayFund | null
  sellerConfirm: OverlayConfirm | null
  buyerConfirm: OverlayConfirm | null
  release: OverlayRelease | null
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): HandoffPayload | null {
...
function handoffOutputIndexes(tx: Transaction): number[] {
...
async function submitBeefFallback(host: string, topic: string, beef: number[]): Promise<SubmitResult> {
  ...
  return {
    admitted: handoffOutputIndexes(tx),
    raw,
    host,
    topic
  }
...
export async function submitHandoffTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupHandoffItems(base: string, query: ListingQuery = {}): Promise<OverlayItem[]> {
  const host = overlayUrl(base)
...
export async function lookupListing(
  ...
  const items = await lookupHandoffItems(base, { listingId, txid })
  return viewFromItems(items, listingId)
}

export async function lookupListings(base: string): Promise<ListingView[]> {
  const items = await lookupHandoffItems(base)
  const ids = [...new Set(
    items
      .filter((item) => item.payload.kind === 'list')
      .map((item) => item.payload.listingId)
  )]
  return ids
...
export function viewFromItems(items: OverlayItem[], listingId?: string): ListingView {
  const scoped = listingId
    ? items.filter((item) => item.payload.listingId === listingId)
    : items
  const lists = scoped.filter((item) => item.payload.kind === 'list') as Array<OverlayItem & { payload: HandoffList }>
  const funds = scoped.filter((item) => item.payload.kind === 'fund') as Array<OverlayItem & { payload: HandoffFund }>
  const confirms = scoped.filter((item) => item.payload.kind === 'confirm') as Array<OverlayItem & { payload: HandoffConfirm }>
  const releases = scoped.filter((item) => item.payload.kind === 'release') as Array<OverlayItem & { payload: HandoffRelease }>

  const list = lists[0]
    ? { ...lists[0].payload, txid: lists[0].txid, outputIndex: lists[0].outputIndex }
    : null
  const fund = funds[0]
    ? { ...funds[0].payload, txid: funds[0].txid, outputIndex: funds[0].outputIndex }
    : null
  const sellerRow = confirms.find((item) => item.payload.party === 'seller')
  const buyerRow = confirms.find((item) => item.payload.party === 'buyer')
  const sellerConfirm = sellerRow
    ? { ...sellerRow.payload, txid: sellerRow.txid, outputIndex: sellerRow.outputIndex }
    : null
  const buyerConfirm = buyerRow
    ? { ...buyerRow.payload, txid: buyerRow.txid, outputIndex: buyerRow.outputIndex }
    : null
  const release = releases[0]
    ? { ...releases[0].payload, txid: releases[0].txid, outputIndex: releases[0].outputIndex }
    : null

  return {
    listingId: listingId || list?.listingId || '',
    list,
    fund,
    sellerConfirm,
    buyerConfirm,
    release,
    status: listingStatus({ list, fund, sellerConfirm, buyerConfirm, release })
  }
}
 ...
 * BEEF output and keep MAGIC handoff fields only.
 */
...
function matchesQuery(item: OverlayItem, query: ListingQuery): boolean {
  ...
  if (query.listingId && item.payload.listingId !== query.listingId) return false
  return true
}
