...
export const OVERLAY_STORAGE_KEY = 'record-desk.overlayUrl'

export const LOCAL_OVERLAY_HINT =
  'Optional local Docker override: cd record-desk && docker compose up --build (overlay :8083, UI :5176), then set Overlay URL to http://localhost:8083 for tm_records / ls_records.'
...
/**
 * Pages never defaults to localhost. Local Vite/Docker may still point at :8083
 * via VITE_OVERLAY_URL or the in-UI overlay URL (custom tm_records).
 */
