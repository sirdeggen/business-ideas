...
describe('hash-list overlay-first read', () => {
  ...
  it('loads overlay rows with no wallet and does not call listOutputs', async () => {
    ...
    expect(result.rows[0].time).toBe(overlayRow.time)
  ...
  it('shows empty only when overlay lookup returned no matching records', async () => {
    const result = await loadRecordsList(OVERLAY_URL, null, {
      inspectLookup: async () => ({ rows: [], listed: 12, parsed: 0, unparsed: [] }),
      inspectHeld: vi.fn(async () => {
        throw new Error(WALLET_MISSING)
      })
    })
