import type { Proposal, Treasury } from './store.js'
import { ROLE_LABEL, shortKey, type Role } from '../../protocol/treasury.js'
...
export function paymentsPdf(treasury: Treasury, yearMonth: string): Buffer {
  const rows = paymentsInMonth(treasury, yearMonth)
  ...
  return Buffer.concat([Buffer.from(body), Buffer.from(xref), Buffer.from(trailer)])
}
