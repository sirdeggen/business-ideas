...
/** Invoice amount in dollars. Never the 1-sat PushDrop token, never “N sats”. */
export function formatInvoiceAmount(amountSats: number, usdPerBsv: number | null): string {
  if (!Number.isFinite(amountSats) || amountSats < 1) return ''
  if (usdPerBsv && usdPerBsv > 0) return formatUsd(satsToUsd(amountSats, usdPerBsv))
  return ''
}
