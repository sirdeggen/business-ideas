...
import { submitClaimTx, txFromWalletBeef, type OverlayClaim } from './overlay'
...
export function mintPriceSats(input: Pick<MintInput, 'priceSats'>): number {
  return input.priceSats
}

export { formatSats }

export function assertCanMint(input: MintInput): void {
  if (!input.label.trim()) throw new Error('Label is required.')
  if (!input.itemSerial.trim()) throw new Error('Write the item or serial.')
  if (input.itemHashNote.length > HASH_NOTE_MAX) throw new Error('Item note too long for v0.')
  const priceError = validatePrice(input.priceSats)
  if (priceError) throw new Error(priceError)
}

export function assertSerialFree(live: OverlayClaim[], issuer: string, itemSerial: string): void {
  if (liveSerialTaken(live, issuer, itemSerial)) throw new Error(SERIAL_TAKEN)
}
...
export async function mintClaim(
  ...
  assertCanMint(input)
  assertSerialFree(live, identityKey, input.itemSerial)
  const timestamp = nowIso()
  const itemHash = resolveItemHash(input.itemHashNote)
  const claimId = makeClaimId(identityKey, input.itemSerial.trim(), timestamp, randomNonce())
  const token = {
    claimId,
    label: input.label.trim(),
    itemSerial: input.itemSerial.trim(),
    itemHash,
    holder: identityKey,
    issuer: identityKey,
  ...
  const response = await wallet.createAction({
    description: `Mint claim: ${token.label}`,
    outputs: [{
  ...
  if (!txid || !tx) {
    throw Object.assign(new Error('Wallet did not return a mint transaction'), { cause: response })
  }

  try {
    await submitClaimTx(overlayUrl, tx)
    return { claimId, txid, itemHash }
  } catch (error) {
...
export async function transferClaim(
  ...
  const next = {
    ...
    itemSerial: held.claim.itemSerial,
    itemHash: held.claim.itemHash,
    holder: recipient,
