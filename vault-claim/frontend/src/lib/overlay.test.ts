...
describe('overlay topic rails', () => {
  ...
  it('does not invent a custom topic on localhost', () => {
    ...
    expect(overlayTopic('http://localhost:5180')).not.toBe('tm_vault')
    expect(overlayLookupService('http://localhost:5180')).not.toBe('ls_vault')
    expect(overlayTopic('http://localhost:5180')).not.toBe('tm_vaultclaim')
  })
})
