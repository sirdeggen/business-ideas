...
export interface OverlayClaim extends ClaimToken {
  txid: string
  outputIndex: number
}

export interface OverlayRedeem extends ClaimRedeem {
  txid: string
  outputIndex: number
}
...
export async function lookupClaims(base: string): Promise<OverlayClaim[]> {
  ...
  const redeems = items
    .filter((item): item is OverlayItem & { payload: ClaimRedeem } => item.payload.kind === 'redeem')
    .map((item) => item.payload)
  return currentClaims(claims, redeems)
}
