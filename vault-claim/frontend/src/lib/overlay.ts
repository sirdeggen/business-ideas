...
export interface OverlayClaim extends VaultClaimView {
  txid: string
  outputIndex: number
}
...
function parseScript(lockingScript: Parameters<typeof PushDrop.decode>[0]): ClaimPayload | null {
...
function claimOutputIndexes(tx: Transaction): number[] {
...
async function submitBeefFallback(host: string, topic: string, beef: number[]): Promise<SubmitResult> {
  ...
  return {
    admitted: claimOutputIndexes(tx),
    raw,
    host,
    topic
  }
...
export async function submitClaimTx(base: string, beef: number[]): Promise<SubmitResult> {
...
export async function lookupClaimItems(base: string, query: ClaimQuery = {}): Promise<OverlayItem[]> {
  const host = overlayUrl(base)
...
export async function lookupClaims(base: string): Promise<OverlayClaim[]> {
  const items = await lookupClaimItems(base)
  const claims = items
    .filter((item): item is OverlayItem & { payload: ClaimToken } => item.payload.kind === 'claim')
    .map((item) => item.payload)
  const redeems = items
    .filter((item): item is OverlayItem & { payload: RedeemReceipt } => item.payload.kind === 'redeem')
    .map((item) => item.payload)
  const views = currentVaultClaims(claims, redeems)
  return views.map((view) => {
    const match = items.find((item) => {
      if (view.status === 'redeemed') {
        return item.payload.kind === 'redeem' && item.payload.claimId === view.claimId
      }
      return item.payload.kind === 'claim' && item.payload.claimId === view.claimId
    })
    return {
      ...view,
      txid: match?.txid ?? '',
      outputIndex: match?.outputIndex ?? 0
    }
  })
}
...
function matchesQuery(item: OverlayItem, query: ClaimQuery): boolean {
  ...
  if (query.claimId && item.payload.claimId !== query.claimId) return false
  return true
}
...
function fromContext(context: number[] | undefined, outputIndex: number): OverlayItem | null {
  ...
  try {
    const parsed = JSON.parse(new TextDecoder().decode(Uint8Array.from(context))) as ClaimPayload & {
      txid?: string
      outputIndex?: number
    }
