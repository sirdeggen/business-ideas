...
export const OVERLAY_STORAGE_KEY = 'trace.overlayUrl'
export const RECEIPT_CACHE_KEY = 'trace.last-good'

export const LOCAL_OVERLAY_HINT =
  'Optional overlay URL override. Topic stays tm_anytx / ls_anytx — this desk does not invent a custom topic.'
...
export const DECLINED_SPEND =
  'You declined the spend. The receipt was not registered.'

export const OVERLAY_ACTION_FAILED =
  'Couldn’t reach the overlay. Try again in a moment.'

/**
 * Pages never defaults to localhost. Local Vite may still point at another
 * host via VITE_OVERLAY_URL or the in-UI overlay URL.
 */
...
export function errorMessage(error: unknown): string {
  ...
  if (looksLikeRejected(formatted) || looksLikeRejected(raw)) return DECLINED_SPEND
